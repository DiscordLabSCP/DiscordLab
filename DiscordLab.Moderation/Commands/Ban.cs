...
public class Ban : ISlashCommand
{
    ...
    {
        string user = command.Data.Options.First(option => option.Name == Translation.BanCommandUserOptionName).Value.ToString();
        string reason = command.Data.Options.First(option => option.Name == Translation.BanCommandReasonOptionName).Value.ToString();
        string duration = command.Data.Options.First(option => option.Name == Translation.BanCommandDurationOptionName).Value.ToString();
        
        string response = Server.ExecuteCommand($"oban {user} {duration} {reason}");
        if (!response.Contains("has been banned"))
        {
            await command.RespondAsync(Translation.FailedExecuteCommand.Replace("{reason}", response), ephemeral: true);
        }
        else
        {
            ...
            {
                ModerationLogs.Handlers.DiscordBot.Instance.SendBanMessage(null, user, reason, $"<@{command.User.Id}>", null, duration);
            }
        }
    }
}